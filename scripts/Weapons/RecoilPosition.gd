...
func add_recoil() -> void:
	target_position += Vector3(randf_range(recoil_amount.x, recoil_amount.x*2.0), randf_range(
		recoil_amount.y, recoil_amount.y*2.0), randf_range(-recoil_amount.z/2, -recoil_amount.z))
